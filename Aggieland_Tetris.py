...
def main(screen):
    ...
    while run:
        ...
        if fallTime / 1000 > (fallCheck / fallSpeedMult):  # in ms 
            fallTime = 0 
            currentPiece.y += 1 #Remember Y goes down so plus not minus.
    ...
    for i in range(20):
        for j in range(10):
            if (j, i) in lockedPositions:
                pointColor = lockedPositions[(j,i)]
                grid[i][j] = pointColor
